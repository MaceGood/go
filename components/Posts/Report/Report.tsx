...
const Report: React.FC<Props> = ({
  ...
  const handleReport = async (): Promise<void> => {
    ...
    await axios
      .post(`http://localhost:3000/api/report?q=${email}`, options)
      .catch((error) => console.log(error.response));
    setOpen(false);
    setReportMessage("");

    // ako e povike od povike od 3 pati probano da se reportne user dadi error message
  };
                    ...
                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                      ...
                      <div className="mt-2">
                        <p className="text-sm text-gray-500">
                          Tell us why are you reporting this user?
                        </p>
                      </div>
                    </div>
                  </div>
